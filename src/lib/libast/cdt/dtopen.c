...
static int _dttype2005(astDt_t* dt, int type)
{
...
#if __STD_C
astDt_t* _astdtopen(astDtdisc_t* disc, astDtmethod_t* meth, unsigned long version)
#else
astDt_t*	_astdtopen(disc, meth, version)
astDtdisc_t*	disc;
astDtmethod_t*	meth;
...
#endif
{
	...
	dt = NIL(astDt_t*);
	data = NIL(astDtdata_t*);
	type = meth->type;

	memset(&pdt, 0, sizeof(astDt_t));
	pdt.searchf = meth->searchf;
	pdt.meth = meth;
	astdtdisc(&pdt,disc,0); /* note that this sets pdt.memoryf */

	if(disc->eventf)
	{	if((ev = (*disc->eventf)(&pdt,DT_OPEN,(Void_t*)(&data),disc)) < 0)
			return NIL(astDt_t*); /* something bad happened */
		else if(ev > 0)
	...
	if(pdt.data->type&DT_INDATA)
		dt = &pdt.data->dict;
	else if(!(dt = (astDt_t*) malloc(sizeof(astDt_t))) )
...
#undef astdtopen /* deal with binary upward compatibility for op bits */
#if __STD_C
astDt_t* astdtopen(astDtdisc_t* disc, astDtmethod_t* meth)
#else
astDt_t*	astdtopen(disc, meth)
astDtdisc_t*	disc;
astDtmethod_t*	meth;
#endif
{
	return _astdtopen(disc, meth, 20050420L);
}

/* below are private functions used across CDT modules */
astDtlink_t* _dtmake(astDt_t* dt, Void_t* obj, int type)
{
	astDthold_t	*h;
	...
	return (astDtlink_t*)h;
}

void _dtfree(astDt_t* dt, astDtlink_t* l, int type)
{
	astDtdisc_t	*disc = dt->disc;
...
int astdtuserlock(astDt_t* dt, unsigned int key, int type)
{
	if(type > 0)
...
Void_t* astdtuserdata(astDt_t* dt, Void_t* data, unsigned int key)
{
	if(key == 0)
		return dt->data->user.data;
	else if(astdtuserlock(dt, key, 1) < 0 )
		return NIL(Void_t*);
	else
	{	dt->data->user.data = data;
		astdtuserlock(dt, key, -1);
		return data;
	}
}
